...
const registerUser = asyncHandler(async (req, res) => {
  ...
  // If we dont upload coverImage then in the above case we will get error so to handle that error
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  //   If we want to login on based of if we get one of email or username then the logic will be like
  // if(!(username || email )){
  // throw new ApiError(400, "Username or email is required")
  // }
...
const changeCurrentPassword = asyncHandler(async (req, res) => {
  ...
  if (!isPasswordCorrect) {
    throw new ApiError(400, "Incvalid old Password");
  }
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  console.log("avatar uploadOnCloudinary (avatar.url):", avatar);

  if (!avatar.url) {
    throw new ApiError(400, "Error While uplading the image");
  }
  const user = await User.findByIdAndUpdate(
    req.user._id,
    {
